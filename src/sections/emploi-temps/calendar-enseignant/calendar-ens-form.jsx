...
export default function CalendarForm({
  ...
  const [selectedClasseSemestreId, setSelectedClasseSemestreId] = useState(null);
  ...
  const logManuallyWeeks = manuallySelectedWeeks;
  console.log("Liste des semaines manuellement sélectionnées dans CalendarForm:", logManuallyWeeks);
  ...
  const methods = useForm({
    ...
    defaultValues: {
      ...
      classeSemestre: selectedClasseSemestreId || '',
    },
  });
  ...
  const { semestreSelectionne } = useGlobalData(); // Récupérez le semestre sélectionné et la liste des semestres
 
  // eslint-disable-next-line no-shadow
 
  const loadModulesByEnsAndYear = async () => {
    try {
      const data = await planningService.getPlanningByEmployeAndSemestre(employeeId, semestreSelectionne,cursusSelectionne);
            console.log("Modules chargés:", data);
      // eslint-disable-next-line no-shadow
      const modules = data.map(item => ({
        id: item.idModule,
        designation: `${item.designation} -${item.nomClasse}`,
        employes: item.employes,
        selectedClasseSemestreId : item.idClasseSemestre,
...
useEffect(() => {
    ...
    if (currentEvent) {
      ...
      setSelectedPlageHoraire(currentEvent.plageHoraire);
  ...
  const handleModuleChange = (event, field) => {
    ...
    if (selectedModule) {
      ...
      setSelectedClasseSemestreId(selectedModule.selectedClasseSemestreId)
      setSelectedEmployeeIds(selectedModule.employes.map(emp => emp.idEmploye));
    } else {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const cours = {
        description,
        datecours: new Date(start).toISOString().split("T")[0],
        classeSemestres: [{ id: selectedClasseSemestreId }],
        modules: [{ id: module || selectedModuleId }],
        employes: [{ id: employeeId }], // Utilisez l'ID de l'employé ici
        idplagehoraire: { id: selectedPlageHoraire }, // Ajoutez l'objet plage horaire
        salles: selectedSalleId ? [{ id: selectedSalleId }] : [] // Ajoutez la salle seulement si elle est sélectionnée
      };
      // Utilisez selectedTeacherId ici
      if (selectedTeacherId) {
        cours.employes.push({ id: selectedTeacherId });
      }
  ...
  >
    ...
    {modules.map((module) => (
      <MenuItem value={module.id} key={module.id}>
            <Tooltip title={`${module.nbHeureAdd} / ${module.chargeH} heures`} key={module.id}>
...
CalendarForm.propTypes = {
  ...
  selectedTeacherId: PropTypes.number.isRequired,
};
