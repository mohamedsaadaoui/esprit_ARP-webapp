...
import RetardTableRow from './retard-table-row';
import RetardableToolbar from './retard-table-toolbar';
// ----------------------------------------------------------------------
...
const TABLE_HEAD = [
  ...
  { id: 'dureeRetard', label: 'Duree de retard ', width :180 },
  { id: '', width: 88 },
...
export default function RetardListView() {
  ...
  const { semestreSelectionne, semestres } = useGlobalData(); // Retrieve selected semester and semester list
  const semestreCorrespondant = findSemestreById(semestreSelectionne, semestres); // Find the corresponding semester
  const { userPermissions } = useAuthContext();

  const [tableData, setTableData] = useState([]);
  const [filters, setFilters] = useState(defaultFilters);
  const fetchData = async () => {
    if (semestreCorrespondant) { // Ensure the corresponding semester exists
      try {
        const retards = await retardService.getRetardsBySemestre(semestreCorrespondant.id); // Use the selected semester ID
        setTableData(retards);
        console.log('Fetched retards:', retards);
      } catch (error) {
        enqueueSnackbar(`Error fetching retards: ${error.message}`, { variant: 'error' });
      }
    }
  };
  ...
  }, [enqueueSnackbar, semestreCorrespondant]); // Update the dependency array to include semestreCorrespondant
  ...
  return (
    <PermissionBasedGuard permissions={['VIEW_RETARD']}hasContent>

      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Liste des retards"
          links={[
            { name: 'Dashboard', href: paths.dashboard },
            { name: 'Retard', href: paths.dashboard.seven },
            { name: 'Liste des retards' },
          ]}
...
"
        content={
          <>
            Êtes-vous sûr de vouloir supprimer ce retard ? <strong> {table.selected.length} </strong> items?
          </>
        }
        action={
          <Button
