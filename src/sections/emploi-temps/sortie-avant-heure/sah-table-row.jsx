...
export default function SahTableRow({ row, selected, onEditRow, onSelectRow, onDeleteRow, refreshedList, userPermissions }) {
  const { id, idcours, idEmploye, dureeSortie, avatarUrl } = row;
  ...
  const handleEdit = () => {
  //  const formattedTime = format(dureeSortie, 'HH:mm:ss');
    const formattedTime = parseTimeStringToDate(dureeSortie)
    setDureeRetard(formattedTime)
    console.log("rrr ",formattedTime)
  ...
  const handleSubmitEdit = async (data) => {
    try {
        const timeString = dureeRetard
            ? `${dureeRetard.getHours().toString().padStart(2, '0')}:${dureeRetard.getMinutes().toString().padStart(2, '0')}`
            : '';
 
        // Attendez la réponse du backend pour récupérer le message
        const response = await sahService.modifierDureeSortie(id, { dureeSortie: timeString });
        refreshedList();
  ...
  const handleDelete = async () => {
    try {
      await sahService.deleteSortieAvantHeure(id);
  ...
  return (
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>
          {formatDurationToMinutes(dureeSortie)} minutes
        </TableCell>
 
        <TableCell align="right" sx={{ px: 1, whiteSpace: 'nowrap' }}>
  {(userPermissions.includes('UPDATE_SORTIE') || userPermissions.includes('DELETE_SORTIE')) && (
    <IconButton color={popover.open ? 'inherit' : 'default'} onClick={popover.onOpen}>
      ...
      <CustomPopover
        ...
        sx={{ width: 140 }}
      >
        {userPermissions.includes('UPDATE_SORTIE') && (
          <MenuItem onClick={handleEdit}>
            <Iconify icon="solar:pen-bold" />
            Modifier
          </MenuItem>
        )}
        {userPermissions.includes('DELETE_SORTIE') && (
          <MenuItem
            onClick={() => {
              confirm.onTrue();
              popover.onClose();
...
"
        content="Êtes-vous sûr de vouloir supprimer cette sortieAH ??"
        action={
